...
mkdir -p "/usr/share/postgresql/$HE3SQL_VERSION"
mkdir -p /usr/include/postgresql

# Remove old bin files — some may be symlinks to the highest PG version
rm -f "/usr/lib/postgresql/$HE3SQL_VERSION/bin/"*

# Copy he3sql bin/ and lib/ into the target (merge with apt contrib .so files)
cp -r /he3sql/bin/*    "/usr/lib/postgresql/$HE3SQL_VERSION/bin/"
cp -r /he3sql/lib/*    "/usr/lib/postgresql/$HE3SQL_VERSION/lib/"
cp -r /he3sql/share/*    "/usr/lib/postgresql/$HE3SQL_VERSION/share/"
